...
namespace ConvertPinYin
{
    public class Pinyin
    {
        ...
        public static string Convert(string hzString, int maxLength)
        {
            ...
            return pyString.Substring(0,1)+pyString.Substring(1,pyString.Length-1).ToLower();
        }


    }

    public class IniPinYin   // 首字母缩写
    {
        public static string GetChineseSpell(string strText)
        {
            int len = strText.Length;
            string myStr = "";
            for (int i = 0; i < len; i++) myStr += getSpell(strText.Substring(i, 1));           
            return myStr;
        }
